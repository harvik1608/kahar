...
<div class="page-header">
    <div class="page-title">
        <h4>{{ $_role == 3 ? 'Vendor' : 'Customer' }} List</h4>
        <h6>(<span class='mandadory'>*</span>) indicates required field.</h6>
    </div>
</div>
@if($_role == 3)
    <form action="{{ is_null($admin) ? url('admin/vendors') : url('admin/vendors/'.$admin->id) }}" method="POST" enctype="multipart/form-data" id="mainForm">
@else 
    <form action="{{ is_null($admin) ? url('admin/customers') : url('admin/customers/'.$admin->id) }}" method="POST" enctype="multipart/form-data" id="mainForm">
@endif
    @csrf
    @if(!is_null($admin))
    ...
    <input type="hidden" class="form-control" name="role" value="{{ $_role }}" />
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4>{{ is_null($admin) ? "New" : "Edit" }} {{ $_role == 3 ? 'Vendor' : 'Customer' }}</h4>
                </div>
                <div class="card-body profile-body">
                    ...
                    <div class="text-end mt-2">
                        ...
                        <a href="{{ $_role == 3 ? url('admin/vendors') : url('admin/customers') }}" class="btn btn-secondary" id="backBtn">Back</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
	var page_title = "{{ $_role == 3 ? 'Vendor List' : 'Customer List' }}";
